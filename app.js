...
function readManualTheme() {
  try {
    ...
    if (saved && ['light', 'dark'].includes(saved.theme) && Number(saved.expiresAt) > Date.now()) {
      return saved;
    }
...
function saveManualTheme(theme) {
  try {
    ...
    // The selected theme still works for the current page view.
  }
}
...
function scheduleThemeCheck() {
  ...
  const delay = Math.max(1000, nextBoundary.getTime() - Date.now() + 750);
...
function createThemeControl() {
  ...
  const headerAction = navWrap.querySelector('.button-header');
  navWrap.insertBefore(themeButton, headerAction || null);
...
document.querySelectorAll('[data-whatsapp]').forEach((link) => {
  ...
  link.href = whatsappUrl(messages[key] || messages.general);
});
